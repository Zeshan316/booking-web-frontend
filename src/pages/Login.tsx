...
import img from '../assets/bus-img.png'
import './Login.css'
...
function Login(): JSX.Element {
	...
	React.useEffect(() => {
		...
		if (token?.length || authData.isLoggedIn) {
			const userRole: string =
				authData.user.role.name.toLocaleLowerCase()
			const navigatePath: GenericObject = {
				[USER_ROLES.Admin.toLocaleLowerCase()]: '/users',
				[USER_ROLES.SysAdmin.toLocaleLowerCase()]: '/users',
				[USER_ROLES.User.toLocaleLowerCase()]: '/rides',
				[USER_ROLES.Driver.toLocaleLowerCase()]: '/rides',
			}

			navigate(userRole ? navigatePath[userRole] : '/profile')
		}
	}, [authData])
